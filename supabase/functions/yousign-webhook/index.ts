...
//
// Sur signature_request.completed : download le PDF signé + audit trail
// depuis Yousign API, upload Storage ({org}/awards/{signed,proofs}/...),
// puis appelle complete_signature_request + transition_award_to_granted_after_signature.
//
// Idempotence garantie côté RPC (status checks early return). Si un webhook
// arrive en double, le 2e est no-op.
//
// Déploiement :
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (eventName === 'signer.link_opened' || eventName === 'signer_request.viewed') {
      ...
      // Idempotence : si déjà COMPLETED, no-op
      if (dbRequest.status === 'COMPLETED') {
        console.log(`[yousign-webhook] Already completed, skipping`);
      ...
      // Upload to Storage
      const signedPath = `${dbRequest.org_id}/awards/signed/${dbRequest.document_id}.pdf`;
      const proofPath = `${dbRequest.org_id}/awards/proofs/${dbRequest.document_id}_proof.pdf`;
      ...
      // RPC complete_signature_request → returns awardId | null
      ...
      // Auto-transition award → GRANTED si lié à un award
      if (awardId) {
        ...
        if (transErr) {
          console.error(`[yousign-webhook] Award transition failed: ${transErr.message}`);
          // On ne fail pas le webhook : la sig request est COMPLETED, l'award
          // sera réconcilié manuellement
        }
      }
    } else {
