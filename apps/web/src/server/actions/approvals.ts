...
import { generateAwardDocument } from './documents';
import { notifyCreatorOfApprovalDecision } from './notifications';
...
async function recordDecisionInternal(
  ...
  if (
    ...
    if (req?.award_id) {
      ...
      // auto-send-for-signature : l'admin garde le contrôle des signataires.
      if (status === 'APPROVED' && result.next_award_status === 'APPROVED') {
        await maybeAutoGenerateDocument(supabase, req.award_id);
      }

      // Module 7 B5 — Notif EMAIL au creator (approval_approved/rejected).
      // Fire-and-forget : silent fail logué, ne bloque pas la réponse.
      const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000';
      void notifyCreatorOfApprovalDecision({
        awardId: req.award_id,
        decision: status,
        approverUserId: user.id,
        reason: comment,
        ...
        console.error(
          `[recordDecisionInternal] notifyCreatorOfApprovalDecision failed for award ${req.award_id}:`,
          err,
        );
      });
    }
...
/**
 * Module 6 B5 — Hook auto-generate document.
 *
 * Lookup plan.auto_generate_document via l'award. Si true, lance
 * generateAwardDocument en fire-and-forget. Toute erreur (lookup ou
 * génération) est captée et auditée — jamais propagée à l'approver
 * pour ne pas bloquer son ack côté UI. Le doc apparaîtra (ou pas) au
 * prochain refresh de la page détail award.
 */
async function maybeAutoGenerateDocument(
  ...
  try {
    const { data: award, error: awardErr } = await supabase
      ...
      .eq('id', awardId)
      .maybeSingle();
    if (awardErr || !award) return;

    const { data: plan, error: planErr } = await supabase
      .from('plans')
      .select('auto_generate_document')
      .eq('id', award.plan_id)
      .maybeSingle();
    if (planErr || !plan?.auto_generate_document) return;

    // Fire-and-forget : on ne block pas la réponse à l'approver.
    // Erreurs auditées via document.auto_generate_failed pour debug.
    generateAwardDocument({ awardId })
      .then((res) => {
        if (!res.ok) {
          void logAuditEvent({
            eventType: 'document.auto_generate_failed',
            resourceType: 'AWARD',
            resourceId: awardId,
            metadata: { error: res.error, plan_id: award.plan_id },
          });
        }
      })
      .catch((err: unknown) => {
        ...
        void logAuditEvent({
          ...
          resourceType: 'AWARD',
          resourceId: awardId,
          metadata: { error: msg, plan_id: award.plan_id },
        });
      });
  } catch (err) {
    // Lookup plan échoué → silent fail. Le doc reste à générer manuellement.
    const msg = err instanceof Error ? err.message : 'unknown';
    void logAuditEvent({
      eventType: 'document.auto_generate_failed',
      resourceType: 'AWARD',
      resourceId: awardId,
      metadata: { error: `lookup failed: ${msg}` },
    });
  }
}
