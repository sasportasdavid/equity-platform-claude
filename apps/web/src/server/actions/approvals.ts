...
// `./documents` est importé dynamiquement dans `maybeAutoGenerateDocument`
// pour casser la chaîne d'imports vers `@/lib/pdf/render` (.tsx). Sans ça,
// les tests Vitest de ce fichier plantent au transform parce que la config
// projet n'embarque pas de plugin React JSX (cf. CLAUDE.md > Tests).
...
async function maybeAutoGenerateDocument(
  ...
  try {
    ...
    // Dynamic import : voir commentaire en haut du fichier (évite de tirer
    // `@/lib/pdf/render.tsx` dans le transform Vitest).
    const { generateAwardDocument } = await import('./documents');
