...
export async function updatePlan(planId: string, input: UpdatePlanInput): Promise<MutationResult> {
  ...
  // Map camelCase Zod input → snake_case DB columns. `autoGenerateDocument`
  // est le seul champ qui diffère ; les autres (name/description/status)
  // matchent déjà la convention DB.
  const { autoGenerateDocument, ...rest } = inputCheck.data;
  const dbPatch = {
    ...rest,
    updated_at: new Date().toISOString(),
    ...(autoGenerateDocument !== undefined ? { auto_generate_document: autoGenerateDocument } : {}),
  };

  const { error } = await supabase.from('plans').update(dbPatch).eq('id', idCheck.data);
