...
import { ExternalLink, FileText, Send, Sparkles, Trash2, X } from 'lucide-react';
...
import { updatePlan } from '@/server/actions/plan-mutations';
...
export function Sandbox({
  ...
  function handleToggleAutoGenerate(planId: string, current: boolean) {
    startTransition(async () => {
      const res = await updatePlan(planId, { autoGenerateDocument: !current });
      if (res.ok) {
        toast.success(`auto_generate_document = ${!current}`);
        router.refresh();
      } else toast.error(res.error);
    });
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Sparkles className="size-5" /> 7. Test auto-generate hook (Module 6 B5)
          </CardTitle>
          <CardDescription>
            Si un plan a <code>auto_generate_document=true</code>, les awards approuvés via le
            workflow Module 5 génèrent un document automatiquement (fire-and-forget côté
            <code>approveDecision</code>). Toggle ci-dessous pour tester rapidement.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {plans.map((p) => (
              ...
              >
                <div className="flex-1">
                  ...
                  <Badge
                    variant={p.auto_generate_document ? 'default' : 'outline'}
                    className="ml-1 text-[10px]"
                  >
                    auto_generate = {p.auto_generate_document ? 'ON' : 'OFF'}
                  </Badge>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleToggleAutoGenerate(p.id, p.auto_generate_document)}
                  disabled={pending}
                >
                  Toggle
                </Button>
            ...
            {plans.length === 0 ? (
              <div className="text-muted-foreground text-sm italic">Aucun plan dans cette org.</div>
            ) : null}
          </div>
        </CardContent>
      </Card>
