...
/**
 * Sandbox /dev/document-engine — Module 6 B2 + B3 + B5.
 *
 * B2 : génération PDF (3 templates), preview iframe.
 * B3 : envoi pour signature Yousign + statuses des sig requests en cours.
 * B5 : toggle plan.auto_generate_document pour tester le hook approveDecision.
 */
export default async function Page() {
  ...
  const [awardsRes, templatesRes, docsRes, sigReqsRes, plansRes] = await Promise.all([
    supabase
      .from('awards')
    ...
    supabase
      .from('plans')
      .select('id, name, plan_type, auto_generate_document')
      .eq('org_id', user.activeOrgId)
      .is('deleted_at', null)
      .order('created_at', { ascending: false })
      .limit(20),
